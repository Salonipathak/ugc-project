...
const isBeautyOrSkincareProduct = (productName: string, productDescription?: string) => {
  const text = `${productName} ${productDescription || ""}`.toLowerCase();
  return /sunscreen|spf|serum|skincare|skin science|moistur|cream|lotion|cleanser|toner|beauty|cosmetic|minimalist|face fluid|pa\+/i.test(
    text,
  );
};
